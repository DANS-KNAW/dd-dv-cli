...
@Command(name = "workflow-delete",
         description = "Deletes a workflow by id",
         mixinStandardHelpOptions = true)
@RequiredArgsConstructor
public class WorkflowDelete implements Callable<Integer> {
    private final WorkflowsApi workflowsApi;

    @Parameters(index = "0", paramLabel = "WORKFLOW_ID", description = "Workflow id")
    private long workflowId;

    @Override
    public Integer call() throws Exception {
        try {
            var response = workflowsApi.deleteWorkflow(workflowId);
